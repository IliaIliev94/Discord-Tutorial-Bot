...
for (const commandFolder of commandFolders) {
	const commandFilesFolder = path.join(commandsFolderPath, commandFolder);
	const commandFiles = fs.readdirSync(commandFilesFolder)
		.filter((file) => file.endsWith('.js'));

	for (const file of commandFiles) {
		const filePath = path.join(commandFilesFolder, file);
